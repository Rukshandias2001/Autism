...
const ParentDashboard = ({ childId = "child123" }) => {
  ...
  const handleViewResults = async () => { // Fetch attempts based on selected filters
    try {
      const res = await fetch(
        `https://express-api-440581871543.us-central1.run.app/speech/attempts?childId=${childId}&category=${selectedCategory}&period=${selectedPeriod}` // Fetch attempts for the child with filters
      );
      const json = await res.json();
