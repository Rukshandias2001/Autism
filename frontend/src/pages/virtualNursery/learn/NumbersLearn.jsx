...
import "../../../styles/virtualNurseyStyles/NumbersLearn.css";
import { useParams } from "react-router-dom";
import ReactPlayer from "react-player";
import numbers from "../../../assets/numbers1.png"
...
export default function NumbersLearn({ topic: topicProp }) {
  const { user } = useAuth();
  const isMentor = user?.role?.toLowerCase() === "mentor";

  const { topic: topicURL } = useParams();
  const topic = (topicProp || topicURL || "numbers").toLowerCase();
  ...
  return (
    <div className="al-page">
     ...
     <aside className="al-chart">
            <div className="al-chart-title">Numbers</div>
            <div className="al-image-container">
              <img src={numbers} alt="Numbers Chart" className="al-image" />
            </div>
          </aside>
