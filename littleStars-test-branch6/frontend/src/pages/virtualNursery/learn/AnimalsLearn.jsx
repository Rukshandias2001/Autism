...
import "../../../styles/virtualNurseyStyles/AnimalsLearn.css";
import { useParams } from "react-router-dom";
import ReactPlayer from "react-player";
import animals from "../../../assets/animals1.png";
...
export default function AnimalsLearn({ topic: topicProp }) {
  const { user } = useAuth();
  const isMentor = user?.role?.toLowerCase() === "mentor";

  const { topic: topicURL } = useParams();
  const topic = (topicProp || topicURL || "animals").toLowerCase();
  ...
  return (
    <div className="al-page">
      ...
      <aside className="al-chart">
        <div className="al-chart-title">Animals</div>
        <div className="al-image-container">
          <img src={animals} alt="Fruits Chart" className="al-image" />
        </div>
      </aside>
